...
- The current frontend migration is intentionally following Coinbase CDS design-system practices: layered tokens, theme objects, generated CSS variables, style-prop translation, Box/Text/Pressable-style primitives, page/card/action primitives, accessibility helpers, media-query helpers, and overlay primitives.
- Before large styling, layout, primitive, overlay, navigation, modal, popover, or animation changes, inspect `documentation/design-principles/learning-from-cds/` and the local `src/design-system/bakery/` implementation. Treat those files as the app's active design-system direction, not optional notes.
- For popovers and floating UI, CDS is the reference model: a low-level positioned engine plus concrete panel/dropdown wrappers. Future dependency candidates include `@floating-ui/react` or `@floating-ui/react-dom` for collision-aware placement and `@radix-ui/react-popover` when Radix interoperability is the better fit. Do not add those packages as a casual one-off; install them only as part of a scoped overlay migration and keep the public Bakery primitive API stable.
- When adding app UI, prefer existing Bakery primitives (`BakeryBox`, `BakeryText`, `BakeryPressable`, `BakeryAction`, `BakeryCard`, `BakeryPage*`, `BakeryPopoverPanel`, `SceneButton`, `SceneActionRow`, `SceneSurface`) over new ad hoc Tailwind/CSS patterns. If a primitive is missing, extend the design system first when that reduces duplication.
...
- For the homepage rotating cookie carousel on mobile, preserve the current stable layout unless the user explicitly asks to revisit it. The `Info` control should open a speech-bubble prompt that reuses the add-to-cart prompt surface styling; do not reintroduce the old receipt/barcode/grass-border concept, and do not move the cookie when the info prompt opens.
